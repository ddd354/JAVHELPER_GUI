...
const PY_SRC_FOLDER = "JavHelper"; // path to the python source
const PY_MODULE = "run.py"; // the name of the main module
...
const getPythonScriptPath = () => {
  ...
  return path.join(__dirname, PY_DIST_FOLDER, 'run/run');
};
...
app.on("ready", function() {
  ...
  sleep.sleep(3);  // sleep while python server spin up
  createMainWindow();
});
...
app.on("quit", function() {
  ...
  // ensure background python server is killed
  if (subpy != null) {
    subpy.kill('SIGINT');
  }
});
